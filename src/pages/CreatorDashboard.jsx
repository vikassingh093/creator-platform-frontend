  ...
  const handleWithdraw = async () => {
    ...
    try {
      await apiClient.post('/creators/withdrawal/request', {
        amount: Number(withdrawAmount), method: withdrawMethod,
        upi_id: upiId || null, bank_name: bankName || null,
        account_number: accountNumber || null, ifsc_code: ifscCode || null,
        account_holder: accountHolder || null,
      })
      setShowWithdrawForm(false)
      setWithdrawAmount(''); setUpiId(''); setBankName('')
    ...
    <div className="min-h-screen bg-[#F8F8F8] pb-24">
        ...
        <div className="mx-4 space-y-4">
          ...
          <div className="bg-white rounded-3xl shadow-sm p-4 border border-[#F0F0F0]">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={fetchActiveChats} className="text-xs text-[#FFA500] font-bold">
                Refresh
              </button>
            </div>
            {activeRooms.length === 0 ? (
                  ...
                  <div key={chat.id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-[#AAAAAA]">
                        ...
                        {' • '}
                        <span className={chat.status === 'active' ? 'text-[#00C851]' : 'text-[#AAAAAA]'}>
                          {chat.status}
                        </span>
                      </p>
                    </div>
        ...
        <div className="mx-4 space-y-4">
          ...
          {showWithdrawForm && (
            <div className="bg-white rounded-3xl p-5 shadow-sm border border-[#F0F0F0]">
              ...
              <div className="mb-3">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`flex-1 py-2 rounded-xl text-sm font-extrabold transition ${
                      withdrawMethod === 'upi' ? 'bg-[#FFC629] text-[#1D1D1D]' : 'bg-[#F8F8F8] text-[#757575]'
                    }`}
                  >
                    📱 UPI
                  </button>
                  <button
                    ...
                    className={`flex-1 py-2 rounded-xl text-sm font-extrabold transition ${
                      withdrawMethod === 'bank' ? 'bg-[#FFC629] text-[#1D1D1D]' : 'bg-[#F8F8F8] text-[#757575]'
                    }`}
                  >
                    🏦 Bank
                  </button>
              ...
              {withdrawError && <p className="text-red-500 text-xs mb-3">{withdrawError}</p>}

              <div className="flex gap-2">
                <button
                  onClick={() => { setShowWithdrawForm(false); setWithdrawError('') }}
                  className="flex-1 bg-[#F8F8F8] text-[#757575] font-extrabold py-3 rounded-2xl text-sm"
                >
                  Cancel
                </button>
                <button
              ...
              <div className="space-y-3">
                {withdrawalHistory.map(req => (
                  <div key={req.id} className="border-2 border-[#F0F0F0] rounded-2xl p-3">
                    ...
                    {req.admin_note && (
                      <p className="text-xs text-[#757575] mt-2 bg-[#F8F8F8] rounded-xl p-2">
                        📝 {req.admin_note}
                      </p>
                    )}
                  </div>
      ...
      {activeTab === 'content' && (
        <div className="mx-4 space-y-4">
          ...
          {showUploadForm && (
            <div className="bg-white rounded-3xl p-5 shadow-sm border border-[#F0F0F0]">
              <h3 className="font-extrabold text-[#1D1D1D] mb-4">📸 Upload Content</h3>
              <div className="space-y-3">
                <input
                  type="text"
                  value={uploadTitle}
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full bg-[#F8F8F8] border-2 border-transparent focus:border-[#FFC629] rounded-2xl px-4 py-3 text-sm outline-none resize-none"
                />
                <div className="flex gap-2">
                  {['image', 'video'].map(t => (
                    <button key={t} onClick={() => setUploadType(t)}
                      className={`flex-1 py-2 rounded-xl text-sm font-extrabold ${
                        uploadType === t ? 'bg-[#FFC629] text-[#1D1D1D]' : 'bg-[#F8F8F8] text-[#757575]'
                      }`}>
                ...
                <div className="flex gap-2">
                  <button onClick={() => setUploadIsFree(true)}
                    className={`flex-1 py-2 rounded-xl text-sm font-extrabold ${
                      uploadIsFree ? 'bg-[#00C851] text-white' : 'bg-[#F8F8F8] text-[#757575]'
                    }`}>
                    🆓 Free
                  </button>
                  <button onClick={() => setUploadIsFree(false)}
                    className={`flex-1 py-2 rounded-xl text-sm font-extrabold ${
                      !uploadIsFree ? 'bg-[#FFC629] text-[#1D1D1D]' : 'bg-[#F8F8F8] text-[#757575]'
                    }`}>
                    💰 Paid
                  </button>
                </div>
                ...
                <label className="block">
                  <div className="w-full bg-[#F8F8F8] rounded-2xl px-4 py-6 text-center cursor-pointer border-2 border-dashed border-[#FFC629]">
                    {uploadFile ? (
                      <p className="text-[#00C851] text-sm font-extrabold">✅ {uploadFile.name}</p>
                    ) : (
                      <>
                        <p className="text-3xl mb-1">{uploadType === 'image' ? '🖼️' : '🎥'}</p>
                        <p className="text-[#AAAAAA] text-sm">Tap to select {uploadType}</p>
                      </>
                    )}
                  </div>
                  <input
          ...
          {myContent.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-[#AAAAAA]">No content uploaded yet</p>
            </div>
          ) : (
            <div className="space-y-3">
              {myContent.map(item => (
                <div key={item.id} className="bg-white rounded-2xl p-3 shadow-sm flex gap-3 border border-[#F0F0F0]">
                  <div className="w-16 h-16 rounded-xl bg-[#FFF8E1] overflow-hidden flex-shrink-0">
                    {item.thumbnail ? (
                  ...
                  <div className="flex-1">
                    <p className="font-extrabold text-[#1D1D1D] text-sm">{item.title}</p>
                    <p className="text-xs text-[#AAAAAA] mt-0.5">
                      {item.is_free ? '🆓 Free' : `₹${item.price}`} • {item.content_type}
                    </p>
                    <span className={`text-xs font-extrabold px-2 py-0.5 rounded-full mt-1 inline-block ${getStatusColor(item.status)}`}>
          ...
          {recentReviews.length === 0 ? (
            ...
            recentReviews.map(review => (
              <div key={review.id} className="bg-white rounded-2xl p-4 shadow-sm border border-[#F0F0F0]">
                ...
                {review.comment && (
                  <p className="text-[#757575] text-sm">{review.comment}</p>
                )}
              </div>
            ))
          )}
